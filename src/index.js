...
const adminRoutingCog = createAdminRoutingCog({
  config,
  pending,
  audit,
  rms,
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (ADMIN_COMMANDS.has(interaction.commandName)) {
        ...
        if (interaction.commandName === "config-channel") return adminRoutingCog.previewChannelConfig(interaction);
        if (interaction.commandName === "config-activity") return adminRoutingCog.previewActivityConfig(interaction);
    ...
    if (interaction.isButton()) {
      ...
      if (decision === "approve" && adminRoutingCog.isConfigurationType(type)) {
        ...
        const response = await adminRoutingCog.approveConfiguration(interaction, result.action);
        pending.complete(claimedActionId);
        claimedActionId = null;
        claimedAction = null;
        return response;

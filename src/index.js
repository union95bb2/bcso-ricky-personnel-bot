...
import { CHANNEL_CONFIG_FIELDS } from "./runtime-config.js";
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (ADMIN_COMMANDS.has(interaction.commandName)) {
        ...
        if (interaction.commandName === "config-channel") return previewChannelConfig(interaction);
        if (interaction.commandName === "config-activity") return previewActivityConfig(interaction);
    ...
    if (interaction.isButton()) {
      ...
      if (decision === "renew") {
        if (type === "config-channel" || type === "config-activity") return interaction.reply({ content: "Routing configuration previews do not renew. Run the command again to create a fresh preview.", ephemeral: true });
      ...
      if (decision === "cancel") {
        const cancellation = pending.take(id, interaction.user.id, action => {
          if ((type === "config-channel" || type === "config-activity") && !isServerAdministrator(interaction.member)) return "Only a server administrator can cancel this configuration preview.";
      ...
      if (decision === "approve" && (type === "config-channel" || type === "config-activity")) {
        const result = pending.take(id, interaction.user.id, action => {
          if (!isServerAdministrator(interaction.member)) return "Only a server administrator can save Ricky routing changes.";
          if (action.type !== type) return "This configuration preview does not match the requested action.";
          return null;
        });
        if (result.error) return interaction.reply({ content: result.error, ephemeral: true });
        claimedActionId = result.action.id;
        claimedAction = result.action;
        const response = await approveConfiguration(interaction, result.action);
        pending.complete(claimedActionId);
        claimedActionId = null;
        claimedAction = null;
        return response;

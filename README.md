...
- `/award-role member:@member role:@role [role-2:@role ... role-5:@role]` records and applies up to five approved qualification or unit roles in one preview/approval. The bot validates every selected role against `AWARDABLE_ROLE_IDS` before applying any of them.
- `/remove-role member:@member role:@role [role-2:@role ... role-5:@role]` uses the same allow-list and preview to remove up to five approved qualification or unit roles together. It cannot remove ranks, PAB, moderation, or elevated roles, and fails closed if any selected role is no longer held.

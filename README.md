...
- `/promotion member:@member target-rank:<rank> date:Today` opens a guided promotion form with a required BCSO target-rank dropdown and the shared date selector. `/award-role`, `/remove-role`, and `/personnel-status` use the same date control.
- PAB reviews and forwards the promotion preview; a different Command member must approve/apply it. Approval adds the configured new rank role while retaining prior rank roles as history, posts the personnel record, announces it, and writes an audit entry.
- Final promotion and demotion records go to `PERSONNEL_RECORDS_CHANNEL_ID`; announcements go to `PROMOTIONS_ANNOUNCEMENTS_CHANNEL_ID`. Both final posts directly mention the member plus the configured full-name PAB and Command roles, so the responsible groups are notified in the correct destinations.
...
- `/award-role member:@member role:@role [role-2:@role ... role-5:@role]` records and applies up to five approved qualification or unit roles in one preview/approval. Each role field is a Ricky-filtered autocomplete containing only the current `AWARDABLE_ROLE_IDS` roles, so rank/divider/admin roles do not clutter the picker. The bot validates every selected role again before applying any of them.
- `/remove-role member:@member role:@role [role-2:@role ... role-5:@role]` uses the same filtered role picker and preview to remove up to five approved qualification or unit roles together. It cannot remove ranks, PAB, moderation, or elevated roles, and fails closed if any selected role is no longer held.
...
4. Invite the bot to the BCSO server. In the server role list, move the **actual role assigned to Ricky Bot**—the role named in `/pab-health` as Ricky Bot's highest assigned role—above every rank or qualification role it must change. In the demo this is normally `BCSO Personnel Bot`; `Ricky Controller` only matters if it is actually assigned to the bot. Keep the bot role below the server owner (bots can never manage the owner) and do not test by assigning the target every copied role. Discord will otherwise reject promotion changes.
   Use the server's existing authorization roles: `Personnel Administration Bureau` for `PAB_ROLE_ID` and `BCSO | Command Staff` for `COMMAND_ROLE_ID`. Do not create duplicate short `PAB` or `Command` roles just for Ricky. Make the existing authorization roles **mentionable** in Server Settings → Roles (or grant Ricky Bot the broader Mention Everyone permission). Ricky Bot uses restricted role mentions for approval pings; `/pab-health` flags non-mentionable or elevated approval roles.
5. Enable Developer Mode in Discord: User Settings → Advanced → Developer Mode. Right-click each required role/channel/server and choose **Copy ID**.
   **Quick Discord ID reminder:** the sandbox/server utility commands `/giveid` and `/id` can surface the Discord ID needed for configuration. When the command/argument picker is open, press **Backspace** to back out of it. These are Discord/server utilities, not Ricky Bot workflow commands.
...
7. PAB runs `/award-role` or `/remove-role`, selecting the required role plus any optional `role-2` through `role-5` entries for a batch certification/unit update, reviews the single private preview, and approves it.
...
- Promotions and demotions are intentionally separate. Promotions add the selected rank and retain prior rank roles as history; demotions remove configured rank role(s) and add the selected lower rank. Both require PAB review followed by a different Command member's approval; the PAB reviewer cannot self-approve. Both re-check Discord hierarchy and manageability at the moment of application.
